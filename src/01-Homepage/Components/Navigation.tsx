    ...
    <div className='fixed z-50 flex flex-col gap-0 justify-center items-center right-8 top-[50%] transform-[translate(0, -50%)'>
      ...
      <div className='flex gap-2 items-center'>
        <div className='flex items-center h-4 w-20 text-right'>
          <ParagraphText
            size='xs'
            className={`text-right w-20 ${inViewSlide === indicatorsMetaData.length ? 'hidden md:block' : 'hidden'}`}
          >
            Country level insights
          </ParagraphText>
        </div>
        <div
          className={`cursor-pointer rounded-full w-5 h-5 border-1 border-[#fff] ${inViewSlide === indicatorsMetaData.length ? 'bg-[#fff]' : 'bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.5),rgba(7,43,78,0.5))]'}`}
          onClick={() => {
